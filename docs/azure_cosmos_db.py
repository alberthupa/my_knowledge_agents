...
def fetch_userdata_container(tenantId, userId):
    try:
        query = (
            f"SELECT * FROM c WHERE c.tenantId = '{tenantId}' AND c.userId = '{userId}'"
        )
        items = list(
            userdata_container.query_items(
        ...
        print(
            f"[ERROR] Error fetching user data for tenantId: {tenantId}, userId: {userId}: {e}"
        )
        raise e
...
def patch_active_agent(tenantId, userId, sessionId, activeAgent):
    try:
        ...
        operations = [{"op": "replace", "path": "/activeAgent", "value": activeAgent}]

        try:
            pk = [tenantId, userId, sessionId]
            userdata_container.patch_item(
                item=sessionId, partition_key=pk, patch_operations=operations
            )
        except Exception as e:
...
def delete_userdata_item(tenantId, userId, sessionId):
    try:
        ...
        if len(items) == 0:
            print(
                f"[DEBUG] No user data found for tenantId: {tenantId}, userId: {userId}, sessionId: {sessionId}"
            )
            return
        for item in items:
            userdata_container.delete_item(
                item, partition_key=[tenantId, userId, sessionId]
            )
            print(
                f"[DEBUG] Deleted user data for tenantId: {tenantId}, userId: {userId}, sessionId: {sessionId}"
            )
    except Exception as e:
        print(
...
def create_containers():
    try:
        ...
        print(f"Container {USERS_CONTAINER} created.")
        ...
        print(f"Container {PURCHASE_HISTORY_CONTAINER} created.")
...
def add_purchase(user_id, date_of_purchase, item_id, amount, product_name, category):
    ...
    try:
        ...
        print(
            f"Purchase already exists for user_id {user_id} on {date_of_purchase} for item_id {item_id}."
        )
...
def process_and_insert_data(
    ...
    def process_entry(entry):
        ...
        try:
            ...
            if (
                vector_field
                and vector_field in entry
                and isinstance(entry[vector_field], str)
            ):
        ...
        for attempt in range(retries):
            try:
                ...
                if "TooManyRequests" in str(e) and attempt < retries - 1:
                    ...
                    print(
                        f"Retry {attempt + 1} for entry {entry['id']} after {wait_time} seconds due to rate limit."
                    )
                    time.sleep(wait_time)
                else:
                    print(
                        f"Failed to insert entry {entry['id']} after {attempt + 1} attempts: {e}"
                    )
...
def main():
    try:
        ...
        purchase_history_container = database.get_container_client(
        ...
        process_and_insert_data(
            str(file_prefix / "purchase_history.json"), purchase_history_container
        )
